...
public final class SkillSettings {
   ...
   /** Buff yang terbuka TEPAT pada level ini (untuk node milestone jalur progres v1.4.0). */
   public List<SkillBuff> buffsUnlockedAt(int level) {
      List<SkillBuff> found = new ArrayList<>(2);

      for (SkillBuff buff : this.buffs) {
         if (buff.unlockLevel() == level) {
            found.add(buff);
         }
      }
