...
public final class W2NSMPCommand implements TabExecutor {
   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
      ...
      switch (args[0].toLowerCase(Locale.ROOT)) {
         ...
         case "commands":
            ...
            break;
         case "fishing":
            if (this.require(sender, "w2nsmp.fishing.admin")) {
               this.fishingAdmin(sender, label, args);
            }
            break;
         default:
      ...
      if (args.length == 1) {
         ...
         return SUBCOMMANDS.stream().filter(sub -> sub.startsWith(typed)).filter(sub -> {
            return switch (sub) {
               ...
               case "fishing" -> sender.hasPermission("w2nsmp.fishing.admin");
               default -> true;
            };
         }).toList();
      }
   ...
   /**
    * /w2nsmp fishing give <pemain> <id-item> [jumlah] - beri item pancing custom (v1.4.0).
    * Item dibuat lewat registry (PDC), jadi hasilnya selalu identik dengan drop alami.
    */
   private void fishingAdmin(CommandSender sender, String label, String[] args) {
      if (this.plugin.fishing() == null) {
         this.plugin.messages().send(sender, "fishing.disabled");
         return;
      }

      String action = args.length > 1 ? args[1].toLowerCase(Locale.ROOT) : "";
      if (!"give".equals(action) || args.length < 4) {
         this.plugin.messages().send(sender, "fishing.admin-usage", "label", label);
         return;
      }

      org.bukkit.entity.Player target = Bukkit.getPlayerExact(args[2]);
      if (target == null) {
         this.plugin.messages().send(sender, "fishing.admin-unknown-player", "player", args[2]);
         return;
      }

      me.w2n.w2nsmp.fishing.FishingItem item = this.plugin.fishing().item(args[3]);
      if (item == null) {
         this.plugin.messages().send(sender, "fishing.admin-unknown-item",
            "item", args[3],
            "list", String.join(", ", this.plugin.fishing().itemRegistry().keySet()));
         return;
      }
      ...
      org.bukkit.inventory.ItemStack stack = this.plugin.fishing().createItem(item, amount);
      java.util.HashMap<Integer, org.bukkit.inventory.ItemStack> leftover = target.getInventory().addItem(stack);
      if (leftover != null) {
      ...
      this.plugin.messages().send(sender, "fishing.admin-given",
         "amount", Integer.toString(amount), "item", item.itemName(), "player", target.getName());
   }
