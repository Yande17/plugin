         ...
         if (event.getWhoClicked() instanceof Player player && holder.isOwner(player)) {
            if (event.getClickedInventory() != null && event.getClickedInventory().equals(top)) {
               if (!event.isShiftClick()
                  ...
                  String key = SettingsMenu.keyAt(this.plugin, holder, event.getRawSlot());
                  if (key != null && !"info".equals(key)) {
                     if ("close".equals(key)) {
                        this.plugin.guiSounds().play(player, SettingsMenu.gui(this.plugin), "click");
                        player.closeInventory();
                     } else if (key.startsWith("category:")) {
                        // v1.4.0: buka sub-halaman kategori.
                        holder.category(key.substring("category:".length()));
                        this.plugin.guiSounds().play(player, SettingsMenu.gui(this.plugin), "click");
                        SettingsMenu.render(this.plugin, top, player);
                     } else if ("back".equals(key)) {
                        // v1.4.0: kembali ke halaman utama kategori.
                        holder.category("");
                        this.plugin.guiSounds().play(player, SettingsMenu.gui(this.plugin), "click");
                        SettingsMenu.render(this.plugin, top, player);
