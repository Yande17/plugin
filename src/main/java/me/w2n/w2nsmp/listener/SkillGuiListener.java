...
import me.w2n.w2nsmp.gui.SkillPathMenu;
import me.w2n.w2nsmp.gui.SkillPathMenuHolder;
...
public final class SkillGuiListener implements Listener {
      ...
      try {
         ...
         try {
            ours = SkillMenu.isOwnerMenu(event.getWhoClicked()) || SkillProgressMenu.isOwnerMenu(event.getWhoClicked())
               || SkillPathMenu.isOwnerMenu(event.getWhoClicked());
   ...
   private void handleClick(InventoryClickEvent event, Inventory top) {
      ...
      if (holder instanceof SkillPathMenuHolder path) {
         if (path.isOwner(player)) {
            this.handlePathClick(event, player, path);
         }
   ...
   private void handleProgressClick(InventoryClickEvent event, Player player, SkillProgressMenuHolder holder) {
      ...
      try {
         ...
         if ("path".equals(key)) {
            // Buka jalur progres (snake path) - halaman tempat level pemain berada.
            player.closeInventory();
            Bukkit.getScheduler().runTask(this.plugin, () -> {
               if (player.isOnline()) {
                  SkillPathMenu.open(this.plugin, player, type, -1);
   ...
   /** Menu jalur (snake path): navigasi halaman, kembali ke menu progres, atau detail node. */
   private void handlePathClick(InventoryClickEvent event, Player player, SkillPathMenuHolder holder) {
      String key = SkillPathMenu.keyAt(this.plugin, holder, event.getRawSlot());
      if (key == null) {
         return;
      }
      ...
      try {
         this.plugin.guiSounds().play(player, SkillPathMenu.gui(this.plugin), "click");
         SkillType type = holder.type();
         ...
         if ("prev".equals(key) || "next".equals(key)) {
            ...
            SkillPathMenu.render(this.plugin, event.getView().getTopInventory(), player, holder);
            return;
         }

         int level = SkillPathMenu.nodeLevel(key);
         if (level > 0) {
            // Node milestone: tampilkan detail buff yang terbuka di level itu di chat.
            SkillService service = this.plugin.skills();
            SkillSettings settings = service == null || type == null ? null : service.settings(type);
            if (settings != null) {
               for (SkillBuff buff : settings.buffsUnlockedAt(level)) {
                  SkillInfo.sendBuffDetail(this.plugin, player, type, buff);
               }
            }
         }
   ...
   public void onQuit(PlayerQuitEvent event) {
      try {
         ...
         SkillPathMenu.markClosed(event.getPlayer(), null);
      } catch (Throwable throwable) {
   ...
   public void onClose(InventoryCloseEvent event) {
      try {
         ...
         SkillPathMenu.markClosed(event.getPlayer(), event.getInventory());
      } catch (Throwable throwable) {
   ...
   private static boolean isMenuInventory(Inventory top) {
      ...
      return holder instanceof SkillMenuHolder || holder instanceof SkillProgressMenuHolder
         || holder instanceof SkillPathMenuHolder
         || SkillMenu.isMenu(top) || SkillProgressMenu.isMenu(top) || SkillPathMenu.isMenu(top);
   }
