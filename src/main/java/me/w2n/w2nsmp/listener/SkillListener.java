...
public final class SkillListener implements Listener {
   ...
   /** Anti-abuse: catat blok yang dipasang pemain supaya memecahnya lagi tidak memberi XP. */
   @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
   public void onPlace(BlockPlaceEvent event) {
      try {
         this.handlePlace(event);
      } catch (Throwable throwable) {
         this.note(throwable, "block-place");
      }
   }
   ...
   private void handlePlace(BlockPlaceEvent event) {
      ...
      // Hanya blok yang memang memberi XP yang dicatat - daftar lain tidak membebani memori.
      for (SkillType type : BLOCK_SKILLS) {
         ...
         if (settings.enabled() && settings.hasBlocks() && settings.blocks().contains(material)) {
            service.notePlacedBlock(block.getLocation());
            return;
         }
      }
   }
   ...
   private void handleBreak(BlockBreakEvent event) {
      ...
      // Anti-abuse (v1.4.0): blok yang baru dipasang pemain tidak memberi XP/proc saat dipecah
      // (anti place-break farming). Catatannya dihapus supaya blok alami di lokasi yang sama
      // tetap normal setelahnya.
      boolean placedByPlayer = false;
      try {
         placedByPlayer = service.wasPlacedByPlayer(block.getLocation());
         if (placedByPlayer) {
            service.forgetPlacedBlock(block.getLocation());
         }
      } catch (Throwable throwable) {
         this.plugin.debug("Skill: gagal memeriksa blok pasangan pemain (" + throwable + ").");
      }
   ...
   private void handleEntityDeath(EntityDeathEvent event) {
      ...
      if (amount > 0.0D) {
         // Anti-abuse (v1.4.0): kill atas PEMAIN yang sama dalam jeda pendek tidak memberi XP
         // (anti farming kill teman/akun alt). Kill mob tidak dibatasi di sini.
         if (!(entity instanceof Player) || service.allowPlayerKillXp(killer.getUniqueId(), entity.getUniqueId())) {
