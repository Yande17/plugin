...
NEW_FILES = [
    ...
    'skill/SkillPath.java', 'gui/SkillPathMenu.java', 'gui/SkillPathMenuHolder.java',
]
...
def main():
    ...
    # Alias kurva v1.4.0: sengaja OPSIONAL (tidak ditulis di config bawaan supaya merge
    # otomatis tidak menimpanya); kode membacanya dengan cadangan skills.curve.*.
    optional_paths = {'skills.xp.base', 'skills.xp.multiplier'}
    missing_cfg = sorted(p for p in used_paths if p not in config and p not in optional_paths)
